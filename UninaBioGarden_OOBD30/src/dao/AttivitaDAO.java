	...
	public class AttivitaDAO extends DAO {
		...
		public boolean UpdateTempoLavoratoAttivita(int idAttivita, Time nuovoTempo) throws SQLException {
		    String sql = "UPDATE attività SET tempoLavorato = ?::interval WHERE idAttività = ?";
		    try (PreparedStatement stmt = connection.prepareStatement(sql)) {
		        // Converti Time in stringa nel formato HH:mm:ss
		        String tempoStr = nuovoTempo.toLocalTime().toString();
		        stmt.setString(1, tempoStr);
		        stmt.setInt(2, idAttivita);
		        int rowsAffected = stmt.executeUpdate();
		        return rowsAffected > 0;
		    }
