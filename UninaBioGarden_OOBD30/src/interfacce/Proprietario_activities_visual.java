...
public class Proprietario_activities_visual extends JFrame {
    ...
    public Proprietario_activities_visual(String username_prop, Controller TheController) {
     ...
     try {
		if ( righe.isEmpty()) {
			 throw new Global_exceptions("Non ci sono attività da visualizzare per il proprietario: " + username_prop);
		 }
	 ...
	 // ordino le righe per nome lotto
	 if (righe == null || righe.isEmpty()) {
		 JOptionPane.showMessageDialog(this, "Non ci sono attività da visualizzare per il proprietario: " + username_prop, "Attenzione", JOptionPane.WARNING_MESSAGE);
		 return;
	}
