        ...
        add_coltura.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                try {
                   ...
                   selectedColtura = selectedColtura.split(" - ")[0]; // Prendi solo il nome della coltura
					int validat= TheController.AddColturaToLotto(username, Lottoname, selectedColtura);
					if (validat==-99) {
						throw new Global_exceptions("", Global_exceptions.Tipo.DB_fault);
					}else if(validat==-1) {
						throw new Proprietario_addons_selection_exceptions( Proprietario_addons_selection_exceptions.Tipo.Culture_maximum_number_reached);
					}
					JOptionPane.showMessageDialog(Free_colture, selectedColtura+ " aggiunta con successo!", "Successo", JOptionPane.INFORMATION_MESSAGE);
					TheController.OpenIstanceOfLottoSelected_closeCaller(username, Free_colture.this, Lottoname);
